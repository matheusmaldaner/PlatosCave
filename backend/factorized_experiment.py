...
async def score_nodes_once(
    ...
    *,
    ...
    async def _run_one(n: Dict[str, Any]):
        ...
        role = str(n.get("role", ""))
        async with sem:
            out[nid] = await score_single_node_once(
                llm,
                node_id=nid,
                node_text=text,
                node_role=role,
...
async def run_factorized_resampling_for_pdf(
    *,
    ...
    def _fail_trial(k: int, m: int, *, err: str, rep: Dict[str, Any]) -> TrialResult:
        return TrialResult(
            k=k,
            m=m,
            success=False,
    ...
    try:
        ...
        for k in range(int(k_dags)):
            ...
            if dag_json is None:
                try:
                    ...
                    for m in range(int(m_node_resamples)):
                        trials.append(_fail_trial(k, m, err="dag_extraction_failed", rep=rep_dict))
                    continue
                except Exception as e:
                    ...
                    dag_val_path.write_text(json.dumps(rep_dict, indent=2, ensure_ascii=False), encoding="utf-8")
                    for m in range(int(m_node_resamples)):
                        trials.append(_fail_trial(k, m, err=f"dag_extraction_exception:{type(e).__name__}", rep=rep_dict))
                    continue
            ...
            if rep_dict.get("errors"):
                for m in range(int(m_node_resamples)):
                    trials.append(_fail_trial(k, m, err="dag_validation_failed", rep=rep_dict))
                continue
            ...
            for m in range(int(m_node_resamples)):
                ...
                if node_results is None:
                    try:
                        ...
                        trials.append(_fail_trial(k, m, err=f"node_scoring_failed:{type(e).__name__}", rep=rep_dict))
                        continue
                ...
                try:
                    ...
                    ok = not (math.isnan(score) or math.isinf(score))
                    trials.append(
                        TrialResult(
                            k=k,
                            m=m,
                            ...
                            error=None if ok else "graph_score_nan",
                        )
                    )
                    if ok:
                    ...
                    trials.append(
                        TrialResult(
                            ...
                            error=f"graph_scoring_failed:{type(e).__name__}: {e}",
                        )
                    )

        # 3) Write CSV + summary
        csv_path = out / "graph_scores.csv"
        header = ["k", "m", "success", "graph_score"]
        with csv_path.open("w", encoding="utf-8") as f:
            ...
            for t in trials:
                f.write(",".join([
                    ...
                    "" if (t.graph_score is None or math.isnan(t.graph_score)) else f"{t.graph_score:.8f}",
                ]) + "\n")
