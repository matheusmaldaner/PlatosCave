...
class RoleTransitionPrior:
    ...
    table: Dict[Tuple[str, str], float] = field(default_factory=lambda: {
        ...
        ("Counterevidence","Claim"): 0.25, ("Counterevidence","Conclusion"): 0.25,
...
class KGScorer:
    ...
    def graph_score(self) -> Tuple[float, Dict[str, float]]:
        ...
        # final score
        # NOTE: With the default GraphScoreWeights, the raw weighted sum tops out at 0.85
        # (sum of positive weights) and bottoms out at -0.15 (sum of negative weights).
        # If you want a strict (0,1) score, we map the raw score to [0,1] using these
        # theoretical bounds, then "squeeze" into (0,1) by a tiny epsilon.
        W = self.graph_w
        score_raw = (W.bridge_coverage * bridge_cov +
        ...
        # Theoretical extrema given each component metric is in [0,1].
        wvec = [W.bridge_coverage, W.best_path, W.redundancy, W.fragility, W.coherence, W.coverage]
        max_raw = sum(w for w in wvec if w > 0.0)   # achieved when corresponding metric=1
        min_raw = sum(w for w in wvec if w < 0.0)   # achieved when corresponding metric=1

        denom = max(1e-12, max_raw - min_raw)
        score01 = (score_raw - min_raw) / denom
        score01 = clip01(score01)

        # Strictly inside (0,1), even if score01 hits 0 or 1.
        eps = 1e-6
        score = eps + (1.0 - 2.0 * eps) * score01
        ...
        details = {
            ...
            "coherence": coh,
            "coverage": cov,
            "graph_score_raw": score_raw,
            "graph_score_01": score01,
            "graph_score": score
